...
'''
def DataToTrain(dir_path,out_path,Format='leveldb',parencent=0.1):
    #第一步，分离数据
   print u'第一步，分离数据'
   ...
   command_train = "%s %s%s %s %s %s"%('convert_imageset.exe','--backend=',Format,train_data,\
   ...
   command_test = "%s %s%s %s %s %s"%('convert_imageset.exe','--backend=',Format,test_data,\
   test_imagelist,test)
   print command_test
